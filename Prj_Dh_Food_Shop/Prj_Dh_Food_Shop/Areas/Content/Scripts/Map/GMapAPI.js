...
viettel.Map = function(mapDiv, opts){
    ...
    if(!VTObjChecker.isNumber(opts.zoom)) {
        opts.zoom = 16;
    }
    if(!VTObjChecker.isLatLngObj(opts.center)) {
        opts.center = new viettel.LatLng(21.0279143,105.844878);
    }
...
viettel.LatLngBounds = function(sw, ne){
    ...
    this.extend = function(point){
        ...
        var extendedBound = new viettel.LatLngBounds();
    ...
    this.union = function(otherLatLngBound){
        ...
        var unionedLatLng = new viettel.LatLngBounds();
...
viettel.PolylineOptions = function(){
    ...
    this.getGPolylineOptions = function(polylineOptions){
        ...
        if (polylineOptions.strokeOpacity != null) {
            gPolylineOptions.strokeOpacity = polylineOptions.strokeOpacity;
            this.strokeOpacity = polylineOptions.strokeOpacity;
        }
        ...
        addListener:function(instance, eventName, handler) {                    
            ...
            if(VTObjChecker.isMapTypeObj(instance)){
                var gHander = function(evt){
                    var mouseEvent = new viettel.MouseEvent(instance);
...
viettel.MouseEvent = function(instance){
    var gMouseEvent;
    this.latLng;
    this.pixel;
...
viettel.Projection = function(){
    ...
    this.fromContainerPixelToLatLng = function(pixel){
        return new viettel.LatLng(projection.fromContainerPixelToLatLng(pixel).lat(), projection.fromContainerPixelToLatLng(pixel).lng());
    }
    this.fromLatLngToContainerPixel = function(latLng){
        return projection.fromLatLngToContainerPixel(latLng.getGLatLng());
    }
}
...
viettel.GeoService = function() {
    ...
    this.getAddress = function(vtLatLng, callback) {
        ...
        gGeocoder.geocode({
            ...
            var vtStatus = viettel.GeoServiceStatus.UNKNOWN_ERROR;
            if (gStatus == google.maps.GeocoderStatus.INVALID_REQUEST) {
                vtStatus = viettel.GeoServiceStatus.INVALID_REQUEST;
            } else if(gStatus == google.maps.GeocoderStatus.OK) {
                vtStatus = viettel.GeoServiceStatus.OK;
            } else if(gStatus == google.maps.GeocoderStatus.REQUEST_DENIED) {
                vtStatus = viettel.GeoServiceStatus.REQUEST_DENIED;
            } else if(gStatus == google.maps.GeocoderStatus.UNKNOWN_ERROR) {
                vtStatus = viettel.GeoServiceStatus.UNKNOWN_ERROR;
            } else if(gStatus == google.maps.GeocoderStatus.ZERO_RESULTS) {
                vtStatus = viettel.GeoServiceStatus.ZERO_RESULTS;
            }
            
            var items = null;
            if (vtStatus==viettel.GeoServiceStatus.OK) {
                items = new Array();
                for (var property in gResults) {
                    var item = new viettel.GeoServiceItem();
                    item.location = vtLatLng;
                    item.name = gResults[property].formatted_address;
                    item.address = gResults[property].formatted_address;
                    item.type = viettel.GeoObjType.ALL;
                    item.fulladdress = "";
                    item.website = "";
                    item.phone = "";
                    item.email = "";
            ...
            var vtResult = {
                items: items,
                total: (items==null)?0:items.length
            };
            callback(vtResult, vtStatus);
        });
    }
    
    this.getLocations = function(arg1, arg2, arg3,  callback) {
        if(VTObjChecker.isString(arg1)) {
            //address
            getLocationsByString(arg1, arg2);
        } else if(VTObjChecker.isLatLngBoundObj(arg1)) {
            //LatLngBounds
            getLocationsByLatLngBounds(arg1, arg2, arg3);
        } else if(VTObjChecker.isLatLngObj(arg1)) {
            //LatLng
            getLocationsByLatLngCircle(arg1, arg2, arg3, callback);
        }
    }
    ...
    var getLocationsByString = function(vtAddress, callback){
        gGeocoder.geocode({
            ...
            var items = null;
            if (vtStatus==google.maps.GeocoderStatus.OK) {
                items = new Array();
                for (var i = 0; i< gResults.length; i++) {
                    var item = new viettel.GeoServiceItem();
                    item.location = new viettel.LatLng(gResults[i].geometry.location.lat(), gResults[i].geometry.location.lng());
                    item.name = gResults[i].address_components[0].long_name;
                    item.address = gResults[i].formatted_address;
                    item.type = viettel.GeoObjType.ALL;
                    item.fulladdress = "";
                    item.website = "";
                    item.phone = "";
    ...
    var getLocationsByLatLngBounds = function(vtLatLngBounds, type, callback) {
    ...
    var getLocationsByLatLngCircle = function(vtLatLng, vtRadius, type, callback) {
        var gCLatLng = new google.maps.LatLng(vtLatLng.lat(), vtLatLng.lng());
        var gsw = viettel.GeometryUtil.computeOffset(gCLatLng, vtRadius*Math.sqrt(2), -135);
        var gne = viettel.GeometryUtil.computeOffset(gCLatLng, vtRadius*Math.sqrt(2), 45);
        ...
        gGeocoder.geocode({
            ...
            if (vtStatus==google.maps.GeocoderStatus.OK) {
                items = new Array();
                for (var i = 0; i< gResults.length; i++) {
                    var pt = new viettel.LatLng(gResults[i].geometry.location.lat(), gResults[i].geometry.location.lng());
                    if (viettel.GeometryUtil.getDistanceBetween(vtLatLng, pt)<=vtRadius) {
			var vtResultItem = new viettel.GeoServiceItem();
			var item = new viettel.GeoServiceItem();
			item.location = pt;
			item.name = gResults[i].address_components[0].long_name;
			item.address = gResults[i].formatted_address;
...
viettel.RoutingService = function() {
    var convertGRouteToVTRoute = function(gRoute) {
        if (gRoute==null) return null;
        var gLeg = gRoute.legs[0];
        var vtRoute = {
            length: gLeg.distance.value,
            duration: gLeg.duration.value,
            start_location: new viettel.LatLng(gLeg.start_location.lat(), gLeg.start_location.lng()),
            start_address: gLeg.start_address,
            end_location: new viettel.LatLng(gLeg.end_location.lat(), gLeg.end_location.lng()),
            end_address: gLeg.end_address        
        }
        var ne = gRoute.bounds.getNorthEast();
        var sw = gRoute.bounds.getSouthWest();
        vtRoute.bounds = new viettel.LatLngBounds(
            new viettel.LatLng(sw.lat(), sw.lng()),
            new viettel.LatLng(ne.lat(), ne.lng())
        );
        
        vtRoute.directions = new Array();
        vtRoute.path = new Array();
        var steps = gLeg.steps;
        for (var i=0; i<steps.length; i++) {
            var step = steps[i];
            for (var j=0; j<step.path.length; j++) {
                vtRoute.path.push(new viettel.LatLng(step.path[j].lat(), step.path[j].lng()));
            }
            var item = {
                ...
                roundAbout: false,
                start_location: new viettel.LatLng(step.start_location.lat(), step.start_location.lng()),
                end_location: new viettel.LatLng(step.end_location.lat(), step.end_location.lng()),
                index: (vtRoute.path.length>0?vtRoute.path.length-1:0),
                instruction: step.instructions
            };
            vtRoute.directions.push(item);
        }
        return vtRoute;
    }
    ...
    this.route = function(vtRequest, callback) {
        ...
        var gRequest = {
            ...
            travelMode: vtRequest.travelMode,
        ...
        gDirectionsService.route(gRequest, function(gResponse, gStatus) {
            var vtStatus = viettel.RoutingStatus.UNKNOWN_ERROR;
            ...
            var vtResponse = null;
            if (vtStatus==viettel.RoutingStatus.OK && gResponse.routes.length>0) {
                var vtRoute = convertGRouteToVTRoute(gResponse.routes[0]);   // Chi lay thang dau tien
                vtResponse = vtRoute;
            }
            callback(vtResponse, vtStatus);
        });
    }
}
//================== End ====================
...
viettel.TravelMode.LORRY     = 10000// trong google chua co truong nay nen de so to do nham    
//================== End ====================
//============== RoutingItem ================
...
function SearchControl(controlDiv, map) {
    ...
    function searchLocation(){
        ...
        geocodeService.getLocations(location, function(vtResults, vtStatus){
            if(vtStatus ==  viettel.GeoServiceStatus.OK){     
                map.setCenter(vtResults.items[0].location)
                // setmarker
                if(marker == null){
                    marker = new viettel.Marker();
                    marker.setMap(map);
                }
                marker.setPosition(vtResults.items[0].location);
...
function DrawingPolyline (_polyline){
    ...
    var createNewPoint = function(latLng){
        ...
        marker.setIcon(new viettel.MarkerImage("./images/polylineMarker.png", null, null, new viettel.Point(5, 5), new viettel.Size(10, 10)))
        markers.push(marker);
    ...
    var createNewInterPoint = function(index1, index2, _indexOfInterPoint){
        ...
        interMarker.setIcon(new viettel.MarkerImage("./images/polylineMarker.png", null, null, new viettel.Point(5, 5), new viettel.Size(10, 10)))
